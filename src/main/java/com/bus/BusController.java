...
//	Login form
	...
	public String addMovie(Model m) {
		...
		m.addAttribute("menu", "uploadMovie");
		return "uploadMovie";
	}
	//About us
		@GetMapping("/about")
...
//	Login process
	...
	@PostMapping("/upload")
	public String uploadMovie(@RequestParam("movie_name") String movieName, @RequestParam("movie_desc") String movieDesc,HttpSession session, Model m) throws IllegalStateException, IOException {

		Customer object = (Customer) session.getAttribute("user");
		 MovieDetails mv = mRepo.getMovieByName(movieName);
		 
		 if(mv == null) {
			 
			 MovieDetails mov = new MovieDetails();
			 mov.setMovieName(movieName);
			 mov.setMovieDetails(movieDesc);
			 mov.setImage("movieX.jpg");
			 mRepo.save(mov);
			 
			m.addAttribute("success", "upload succeed");
			return "uploadMovie";
		 } else {
			 m.addAttribute("failed", "upload failed , Movie already exists");
			return "uploadMovie";
		 }
		
		/*
		 * if (object != null) { return "login"; } else {
		 */
...
//	Dashboard page
	...
	public String getUser(@RequestParam("movieName") String movieName, HttpSession session, Model m) {
		...
		if (checkMovie.contains(movieName)) {
			...
			List<String> seatNo1 = new ArrayList<String>();
			Set<String> seatNo2 = new HashSet<String>();
			List<Seat> seat = customer.getSeat();

			List<Seat> all = dao.getAllSeat(now, time);
			
			MovieDetails movie = mRepo.getMovieByName(movieName);
			Set<Seat> mSeats = movie.getSeats();
			...
			for (String  sno : seatNo1) {
				System.out.println(sno);
			}
			
			System.out.println("----------------------------------------");
			
			if(mSeats!= null && !mSeats.isEmpty()) {
				for (String  sno : seatNo1) {
					for(Seat s : mSeats) {
						for(String st : s.getSeatNo()) {
							System.out.println(sno + " == " + st);
							boolean flag = sno.equals(st);
	...
	public String bookSeat(@ModelAttribute("Seat") Seat seat, @RequestParam("movieName") String movieName,
		...
		if (object == null) {
			...
			if (((date.isAfter(currentDate)) || (date.equals(currentDate)))
				...
				MovieDetails movie = mRepo.getMovieByName(movieName);
				dao.saveSeat(seat, object, date2, time, movie);
